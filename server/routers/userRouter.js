...
router.post("/", async (req, res) => {
  try {
    ...
    if (age < 16)
      return res
        .status(400)
        .json({ errorMessage: "You must be of an age 16 or higher" });
    ...
    if (!password.match(/[\W_]/))
      return res
        .status(400)
        .json({ errorMessage: "Password must contain special symbol" });

    if (!password.match(/\d/))
      return res
        .status(400)
        .json({ errorMessage: "Password must contain a number" });
